...
//				.orElseThrow(() -> new AccountNotExistException("Account with id: " + transfer.getAccountFromId() + " does not exist.", ErrorCodes.ACCOUNT_ERROR));
		...
		if(accountFrom.getBalance().compareTo(transfer.getAmountTransferred()) < 0) {
			String errorMessage = "Account with id: " + accountFrom.getAccountId() + " does not have enough balance to transfer.";
			log.info(errorMessage);
			throw new OverDraftException("Account with id: " + accountFrom.getAccountId() + " does not have enough balance to transfer.", ErrorCodes.ACCOUNT_ERROR);
		}
...
//		FROM: Updating the balance of the first account giving money $
		...
		log.info("log -- Balance After Transation for Account id: {} is {} euri.", 
				accountFrom.getAccountId(), accountFrom.getBalance(), " - ",accountFrom.getBalance());
...
//		TO: Updating the balance of the second account receiving money $
        ...
        log.info("log -- Balance After Transation for Account id: {} is {} euri.", 
        		accountTo.getAccountId(), accountTo.getBalance(), " - ", accountTo.getBalance());

  }

  @Override
  public void notifyAboutTransfer(Account account, String transferDescription) {
	// TOBE implement by someone else
  }
}
